...
if command -v xcodegen &> /dev/null; then
    ...
    echo "  xcodebuild -project app/DontBeAFK.xcodeproj -scheme DontBeAFK -configuration Release build"
    exit 0
fi
...
if [ -d "DontBeAFK.xcodeproj" ]; then
    ...
    if xcodebuild -project DontBeAFK.xcodeproj -list &> /dev/null; then
        ...
        if [[ "$response" =~ ^[Yy]$ ]]; then
            ...
            echo "Please fix the project manually or see docs/development/SETUP_XCODE.md"
            exit 1
        fi
    fi
fi
...
echo "  ./scripts/build/setup-project.sh"
echo ""
echo "Option 2: Create project manually in Xcode"
echo "  See docs/development/SETUP_XCODE.md for detailed instructions"
echo ""
...
echo "  4. Save in the app/ directory"
echo "  5. Replace default files with files from app/DontBeAFK/ directory"
echo ""
